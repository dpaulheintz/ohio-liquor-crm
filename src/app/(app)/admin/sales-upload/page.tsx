...
import { getBailmentEntries } from '@/app/actions/bailment';
...
export default async function SalesUploadPage() {
  ...
  let bailmentEntries: BailmentEntry[] = [];
  try {
    bailmentEntries = await getBailmentEntries();
  } catch {
    bailmentEntries = [];
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-1">
      ...
      <SalesUploadClient health={health} bailmentEntries={bailmentEntries} />
    </div>
  );
}
