...
function ChangeChip({ val }: { val: number | null }) {
  if (val === null) return <span className="text-zinc-500 text-xs">—</span>;
...
function KpiCard({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-xl border border-[#C5A572]/20 bg-[#111111] p-5 flex flex-col gap-3 opacity-0 animate-[fadeSlideIn_0.5s_ease_forwards]"
    ...
    >
      ...
      <div className="flex flex-wrap gap-3 text-xs text-zinc-500">
        <span className="flex items-center gap-1.5">
          ...
          <span className="text-zinc-600">{formatter(stats.ly)}</span>
        </span>
        <span className="flex items-center gap-1.5">
          vs 2LY <ChangeChip val={vs2LY} />
          <span className="text-zinc-600">{formatter(stats.twoLY)}</span>
        </span>
      </div>
    </div>
  );
...
export function RevenueOverview({ revenue, bottles, bestMonth, lastUpdated }: OverviewProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      >
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#C5A572] to-transparent" />
        <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Best Month</p>
        {bestMonth ? (
          <>
            <p className="text-3xl font-bold text-white tracking-tight">{fmtDollar(bestMonth.value)}</p>
      ...
      >
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#C5A572] to-transparent" />
        <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Last Updated</p>
        {lastUpdated ? (
          <>
            <p className="text-3xl font-bold text-white tracking-tight">{fmtMonth(lastUpdated)}</p>
            <p className="flex items-center gap-1.5 text-xs text-zinc-500">
              <Calendar className="h-3.5 w-3.5 text-[#C5A572]" />
              Most recent month loaded
            </p>
          </>
        ) : (
          <p className="text-zinc-600 text-sm">No data loaded</p>
        )}
      </div>
    </div>
  );
