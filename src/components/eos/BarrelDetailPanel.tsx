    ...
    <>
      ...
      <div
        className={cn(
          ...
          'bg-[#0f0f0f] border-l border-zinc-800 shadow-2xl',
        ...
        {form && barrel && (
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 shrink-0">
              ...
              <button
                onClick={onClose}
                className="text-zinc-500 hover:text-zinc-200 transition-colors w-7 h-7 flex items-center justify-center text-xl"
              >
                ×
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-transparent border-0 border-b border-zinc-700 pb-1 text-white text-lg font-semibold focus:outline-none focus:border-green-600 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-zinc-500 mb-1.5 block">Quarter</label>
                  <input
                    type="text"
                    value={form.quarter}
              ...
              <div>
                ...
                <div className="space-y-1">
                  {milestones.map(m => (
                    <div
                      key={m.id}
                      className="flex items-center gap-2 group/milestone rounded-lg px-2 py-1.5 hover:bg-zinc-800/50"
                    >
                      <button
                      ...
                      {editingMilestoneId === m.id ? (
                        <input
                          ...
                          className="flex-1 bg-transparent text-sm text-white focus:outline-none border-b border-zinc-600"
                        />
                      ) : (
                        <span
                          onClick={() => {
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover/milestone:opacity-100 text-zinc-700 hover:text-red-400 transition-all text-xs px-1"
                ...
                {addingMilestone ? (
                  <div className="flex items-center gap-2 mt-2 px-2">
                    <div className="w-4 h-4 shrink-0 rounded border border-zinc-600" />
                    <input
                      autoFocus
                      ...
                      className="flex-1 bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-sm text-white focus:outline-none focus:border-green-600"
                  ...
                  <button
                    ...
                    className="mt-2 px-2 text-sm text-zinc-600 hover:text-zinc-400 transition-colors"
                  >
                    + Add Milestone
                  </button>
                )}
            ...
            {/* Footer actions */}
            <div className="shrink-0 px-6 py-4 border-t border-zinc-800 space-y-3">
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full py-2.5 rounded-lg bg-[#2a5a3a] hover:bg-[#3a6a4a] disabled:opacity-50 text-white text-sm font-medium transition-colors"
              >
                {saving ? 'Saving…' : 'Save Changes'}
              </button>
              <button
                onClick={handleDelete}
                className="w-full text-center text-sm text-red-500 hover:text-red-400 transition-colors py-1"
              >
                Delete Barrel
              </button>
            </div>
