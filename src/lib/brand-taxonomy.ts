// ─── Brand Taxonomy ───────────────────────────────────────────────────────────
// Single source of truth for brand codes used by both the admin UI (browser)
// and the Node.js ingestion scripts (scripts/brand-taxonomy.js mirrors this).

export interface BrandInfo {
  brand_family: string;
  product_name: string;
  size: string;
  sub_product?: string; // only set for discontinued / special items
}

export const BRAND_CODES: Record<string, BrandInfo> = {
  // Vodka
...
export const HB_AGENCIES: Record<string, { is_hb_agency: true; hb_location: string }> = {
...
export function resolveBrand(
  ...
  // Unrecognized — derive size from last character
...
/** Strip commas from amount strings like "2,588.76" then parse as float */
...
export function parseBottles(raw: string | number | null | undefined): number | null {
  if (raw === null || raw === undefined || raw === '') return null;
